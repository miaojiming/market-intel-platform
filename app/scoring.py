...
- opportunity_strength 商机强度:对收单独立部署服务商(SI)的销售线索价值。
  8-10分(直接项目): 银行收单系统正式招标RFP/核心系统更换含收单模块/合约到期启动选型/大客户收单系统即将或正在部署
  5-7分(间接机会): 银行核心系统改造可能延伸收单/SaaS厂商进入泰国可能需本地集成/监管政策具体实施引发系统改造/RFI需求征询(非正式招标)/供应商已选定的项目(对其他SI是间接机会)
  2-4分(微弱信号): SaaS厂商获牌/产品发布/一般合作/框架性政策/试点概念验证/已完成或已签约的项目(销售窗口已过)
  1分(无价值): 非收单/纯背景分析/会议花絮/一般行业动态
  状态修正: 已完成/已签约/供应商已确定的项目,在上述基准上降2-3档(如:已完成的直接项目→3-4分,供应商已确定的直接项目→5-6分)
...
- 监管政策:框架/意见征询阶段=微弱信号(商机2-4);有具体实施期限和明确合规要求的=间接机会(商机5-7)
- RFI(需求征询)≠正式招标,归数字化转型规划且商机≤7
- 项目状态关键词: 已完成/已交付/已上线/已签约→商机降档; 供应商已确定/已选定→对其他SI为间接机会; 招标中/选型中/即将启动→直接项目
